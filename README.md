...
2. Исследовать ассемблерный код, генерируемый компилятором GCC с разными уровнями оптимизации.
3. Научиться создавать Makefile и работать с Git.
4. Реализовать многопоточное вычисление с синхронизацией доступа к общему ресурсу (файл).

## Ход работы

## Для начала мы создали программы на C++ для вычисления факториала, а именно: factorial.cpp; factorial.h; 
## Затем была создана программа с параллельным потоком: paral.cpp; 
## Он вычисляет факториал и записывает в файл res.tet (конечный результат). Вычисление факториала от 1-20

## Генерация ассемблерного кода. Файлы: factorial_O0.s; factorial_O1.s; factorial_O2.s; factorial_O3.s;
